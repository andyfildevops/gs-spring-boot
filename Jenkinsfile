    ...
    stages {
        ...
        stage('Deploy to EC2') {
            steps {
                ...
                ssh -i ~/.ssh/id_rsa $DEPLOY_HOST "pkill -f 'java -jar' || true"
                ssh -i ~/.ssh/id_rsa $DEPLOY_HOST "nohup java -jar $DEPLOY_DIR/$JAR_NAME --server.address=0.0.0.0 --server.port=8080 > $DEPLOY_DIR/app.log 2>&1 &"
                '''
            }
}
    }
